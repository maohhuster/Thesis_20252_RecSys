...
# [Mac sanity-check patch] Prefer MPS, fall back to CPU
if torch.cuda.is_available():
    ...
    device = 'cpu'
print(f"[lm_encoding patch] device = {device}")
...
def main(knowledge_path, data_path, model_name, batch_size, aggregate_type):
    ...
    # [Mac sanity-check patch] half() requires fp16 kernels (CUDA only); use float32 on MPS/CPU.
    base = AutoModel.from_pretrained(checkpoint,  trust_remote_code=True)
    if device == 'cuda':
        model = base.half().cuda()
    else:
