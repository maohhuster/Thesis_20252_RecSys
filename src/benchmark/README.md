...
The selection forms a **controlled hierarchy** that answers 7 research questions (consistent with paper §4.2): pre-specified ML-20M ablations (Q1–Q4), cross-density synthesis (Q5–Q6), and robustness (Q7).
...
| Q5 | R1-gene / R1-plus vs M7 across 4 densities | Does paradigm choice depend on per-item density? **(Central finding)** Regularizers (R1-gene, R1-plus) gain monotonically with sparsity; capacity replacers (R2, R3) never beat M7 and collapse when sparse; additive injection (M4/M7) is density-robust. |
| Q6 | SASRec vs M7/M1 across 4 densities | Does the sequential paradigm's density decay differ from the content paradigms? (Steepest dense→sparse endpoint drop.) |
| Q7 | e5-large-v2 encoder; GPT-4o-mini retrain | Are the findings robust to embedding encoder and LLM provider? |
...
### Tier 3: LLM-for-RecSys methods (paradigm classes)

These instantiate two LLM-for-RecSys **paradigm classes**, each triangulated by **two structurally-different instantiations** so claims are class-level: the **regularizer** class (R1, R1-plus — an auxiliary loss aligns CF embeddings toward LLM-semantic structure; the ID embedding is kept) and the **replacer** class (R2, R3 — the ID embedding is replaced by a transform of LLM content, optionally gated). Additive **injection** is the M4/M7 Tier-2 host; SASRec is the orthogonal sequential axis.

#### R1-plus — RLMRec-plus (regularizer, contrastive) (Wei et al., WWW 2024)
...
#### R1 — RLMRec-gene (regularizer, generative) (Wei et al., WWW 2024)
...
**Why included.** R1 (generative) and R1-plus (contrastive) are the **two regularizer instantiations**; running both triangulates the regularizer class across all four density datapoints. This is the paper's headline class: the regularizer-vs-injection gap is monotone in 1/density for *both* instantiations — losing to M7 on dense ML-20M, winning by +26–30% at the sparse Amazon extreme.

#### R2 — KAR-style MoE replacer (HEA adapter, inspired by Xi et al., KDD 2024)
...
**Why included.** R2 is the **gated** replacer instantiation. We adapt KAR's hybrid-expert adapter (originally a CTR method) to a BPR backbone as a representative replacer — it *replaces* the ID embedding with a CF-gated MoE transform of LLM content (this is not a published-KAR reproduction; see the KAR sanity check in the paper appendix).

#### R3 — HypernetReplacer (replacer, gating-free)

**Algorithm.** A paper-original minimal replacer: `e_i = MLP(c_i)` maps LLM content directly to the item embedding with **no per-item ID embedding and no gating**. It is the gating-free counterpart to R2's CF-conditioned MoE.

**Why included.** R2 (gated MoE) and R3 (gating-free) are the **two replacer instantiations**; running both isolates the gating mechanism and triangulates the replacer class. Both *lose* to M7 at every density and collapse when sparse; the R2−R3 sign reverses and localises to the densest point alone (R2 > R3 only on ML-20M), showing MoE-with-CF-gating is a bidirectional CF-signal amplifier, not a one-way loss-amplifier.
...
- **5 random seeds** (42, 123, 456, 789, 2024) controlling parameter initialization and negative sampling
...
Two paradigm classes, each triangulated by **two structurally-different instantiations** so claims are class-level, not method-specific (mechanisms detailed in the Tier-3 section above).

| ID | Model | Class | Integration mechanism | Features |
|----|-------|-------|-----------------------|----------|
| R1 | RLMRec-gene | Regularizer | Generative aux-loss: reconstruct LLM embedding from masked CF embedding (ID kept) | LLM embeddings |
| R1-plus | RLMRec-plus | Regularizer | Contrastive aux-loss: InfoNCE CF↔LLM alignment (ID kept) | LLM embeddings |
| R2 | KAR-style MoE replacer | Replacer | 4-expert MoE with CF-conditioned gating; replaces the ID embedding | LLM profile + mood |
| R3 | HypernetReplacer | Replacer | Gating-free `e_i = MLP(c_i)`; no per-item ID, no gating | LLM profile + mood |

**The question Tier 3 answers (Q5–Q6):** which LLM-for-RecSys paradigm class wins, and does it depend on per-item interaction density? The paper's central finding is **yes** — auxiliary-loss **regularizers** gain monotonically with sparsity, capacity **replacers** never beat M7 and collapse when sparse, and additive **injection** (M4/M7) is density-robust — established at the **class level** via two instantiations per class.

### Key Comparisons

The 7 research questions (matches paper §4.2 and the hierarchy table above):
...
All ML-20M, ML-1M, and Amazon-Books-2018 sweeps are complete (5 seeds × 16+ configurations each).
For headline results see the paper's Table 4 (ML-20M) and Appendix E.13 (cross-density).
For protocol details (200 epochs / patience 20 for LightGCN-SF infrastructure;
upstream-controlled budgets for R1, SASRec, KAR sanity check) see paper §4.3.
...
This layout keeps every encoder variant of a given config adjacent on disk — ideal for the encoder-sensitivity analysis that is a central research question in the paper (§6.4). Comparing bge vs e5 for M4 is a single glob: `results/m4/*/seed-42/results.json`.
