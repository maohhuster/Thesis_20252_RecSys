#!/usr/bin/env python3
"""Re-evaluate R1 (RLMRec-plus) on ML-1M with our `compute_metrics`.
...
Mirror of `scripts/eval_amazon_r1plus.py`, paths swapped to ML-1M.
...
CK_DIR = BENCH / "checkpoints_ml1m" / "r1plus" / "bge-large-en-v1.5"
RESULTS_DIR = BENCH / "results_ml1m"
SEEDS_DEFAULT = [42, 123, 456, 789, 2026]
TOP_K = [10, 20, 50]

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s")
logger = logging.getLogger("eval_r1plus_ml1m")
...
def evaluate_seed(seed: int, layer_num: int, device: str, data, adj):
    ...
    if not ck.exists():
        ...
        alt = BENCH / "checkpoints_ml1m" / "r1plus" / "bge-large-en-v1.5" / f"seed-{seed}" / "best_model.pt"
        if alt.exists():
            ck = alt
        else:
...
def main():
    ...
    print("\n=== R1 (RLMRec-plus) on ML-1M, our compute_metrics ===")
    for k in keys:
        s = summary[k]
        print(f"  {k:10s} {s['mean']:.4f} ± {s['std']:.4f}")

    out_path = RESULTS_DIR / "r1plus_ml1m_metrics.json"
