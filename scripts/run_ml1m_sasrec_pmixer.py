#!/usr/bin/env python3
"""Run pmixer's exact SASRec model on ML-1M with FULL-RANKING eval.

Mirrors `scripts/run_ml20m_sasrec_pmixer.py` (same ML-family hparams) but
points at our ML-1M splits (`processed_ml1m/`).

pmixer's published ML hyperparameters (Result_Norm.md):
    embed_dim    = 50
    num_blocks   = 2
    num_heads    = 1
    dropout      = 0.2      ← ML default; Amazon would use 0.5
    maxlen       = 200      ← ML default; Amazon would use 50
    lr           = 1e-3
    batch_size   = 128
    num_epochs   = 1000
    l2_emb       = 0.0
    optimizer    = Adam, betas=(0.9, 0.98)
    norm_first   = False    ← pmixer default Post-LN

Evaluator: our `evaluate.compute_metrics` (full-ranking) — directly comparable
with M1/M4/M7/R1/R2 on ML-1M.

Usage:
    python3 scripts/run_ml1m_sasrec_pmixer.py --seed 42 --device cuda
"""
...
# pmixer's published ML hyperparameters
...
logger = logging.getLogger("sasrec_pmixer_ml1m")
...
def random_neq(low, high, s):
    ...
    while t in s: t = np.random.randint(low, high)
    return t
...
def sample_one(uid, user_train, n_items, maxlen):
    ...
    nxt = history[-1]; idx = maxlen - 1; ts = set(history)
...
def train(args):
    ...
    hp = SimpleNamespace(**vars(HPARAMS)); hp.device = device
    ...
    out = {
        ...
        "config": {**vars(HPARAMS), "seed": args.seed, "best_epoch": best_epoch,
                   ...
                   "split": "80/10/10 percentile temporal (processed_ml1m)"},
