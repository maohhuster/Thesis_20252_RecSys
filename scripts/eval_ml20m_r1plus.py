#!/usr/bin/env python3
"""Evaluate R1 (RLMRec-plus) checkpoints on Amazon-Books-2018 test set.

Loads each saved best_model.pt (RLMRec format), reconstructs the
LightGCN-gene inference path (symmetric-normalized bipartite adjacency,
sum over `layer_num` propagation layers), scores all (user, item) pairs,
and computes NDCG@10, Recall@10, MRR with our standard
`evaluate.compute_metrics` for direct comparability with M1/M4/M7/R2.

Usage:
    python3 scripts/eval_ml20m_r1.py
    python3 scripts/eval_ml20m_r1.py --layer-num 3 --device cpu
"""
...
DATA_DIR = BENCH / "data" / "processed"  # ML-20M is the base dataset: unsuffixed paths
CK_DIR = BENCH / "checkpoints" / "r1plus" / "bge-large-en-v1.5"
RESULTS_DIR = BENCH / "results"
SEEDS = [42, 123, 456, 789, 2026]
TOP_K = [10, 20, 50]

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s")
logger = logging.getLogger("eval_r1")
...
@torch.no_grad()
def evaluate_seed(seed: int, layer_num: int, device: str, data, adj, ckpt_dir: Path | None = None):
    ck = (ckpt_dir or CK_DIR) / f"seed-{seed}" / "best_model.pt"
...
def main():
    ...
    ap.add_argument("--layer-num", type=int, default=3,
                    help="Matches the upstream `lightgcn_plus.yml > model.ml20m > layer_num: 3` "
                         "(consistent with the original RLMRec ML-20M preset). "
                         "Use 2 only for the depth-control sweep.")
    ap.add_argument("--device", default="cpu")
    ap.add_argument("--seeds", type=int, nargs="+", default=SEEDS,
                    help="Seeds to evaluate (default: all 5). Use a single seed for "
                         "incremental evaluation as the orchestrator finishes each seed.")
    ap.add_argument("--checkpoints-dir", type=Path, default=None,
                    help=f"Override the checkpoint directory. Default: {CK_DIR} "
                         f"(the paper-canonical d=128 location). Pass an alternative path "
                         f"to evaluate superseded checkpoints (e.g., a local d=32 archive); "
                         f"embedding_size is auto-inferred from the checkpoint's "
                         f"user_embeds shape.")
    ap.add_argument("--output", type=Path, default=None,
                    help=f"Override the output metrics JSON path. Default: {RESULTS_DIR/'r1plus_metrics.json'}.")
    args = ap.parse_args()
    ...
    adj = build_norm_adj(data.n_users, data.n_items, data.train_user_items,
                         args.device)
    logger.info(f"adj nnz={adj._nnz()}")

    per_seed = {}
    for seed in args.seeds:
        m = evaluate_seed(seed, args.layer_num, args.device, data, adj, args.checkpoints_dir)
        per_seed[seed] = m
        logger.info(f"seed={seed}  NDCG@10={m['NDCG@10']:.4f}  "
                    f"Recall@10={m['Recall@10']:.4f}  MRR={m['MRR']:.4f}")

    # Aggregate (only over the seeds we actually ran)
    ...
    print(f"\n=== R1-plus (RLMRec-plus) on ML-20M (seeds {args.seeds}) ===")
    for k in keys:
        s = summary[k]
        if "mean" in s:
    ...
    # Per-seed metrics also written to each seed's marker JSON for incremental tracking
    marker_dir = BENCH / "results" / "r1plus" / "ml20m"
    ...
    # Aggregate file (over all seeds passed in)
    out_path = args.output if args.output else (RESULTS_DIR / "r1plus_metrics.json")
