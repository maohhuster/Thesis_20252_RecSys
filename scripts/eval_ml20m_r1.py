#!/usr/bin/env python3
"""Evaluate R1 (RLMRec-gene) checkpoints on the ML-20M test set.
...
`evaluate.compute_metrics` for direct comparability with M1/M4/M7/R1-plus.

R1-gene and R1-plus share an identical LightGCN inference path (the
gene/plus difference is only the auxiliary *training* loss); this script
is the direct sibling of `eval_ml20m_r1plus.py`, pointed at the R1-gene
checkpoints. The ML-20M 54-point selection winner is
layer_num=3, mask_ratio=0.1, recon_weight=0.1, re_temperature=0.2
(reg_weight fixed 1e-7); see code/benchmark/hparams/r1/grid_selection.json.

Reproduction: retrain via `scripts/run_r1_ml20m_single.py` (RLMRec
upstream) for the 5 seeds, dropping each best_model.pt under
code/benchmark/checkpoints/r1/bge-large-en-v1.5/seed-<seed>/, then run
this script.

Usage:
    python3 scripts/eval_ml20m_r1.py
    python3 scripts/eval_ml20m_r1.py --layer-num 3 --device mps
"""
...
CK_DIR = BENCH / "checkpoints" / "r1" / "bge-large-en-v1.5"
...
def main():
    ...
    ap.add_argument("--layer-num", type=int, default=3,
                    help="Matches the ML-20M 54-point selection winner "
                         "(lightgcn_gene.yml > model.ml20m > layer_num: 3). "
    ...
    print(f"\n=== R1 (RLMRec-gene) on ML-20M (seeds {args.seeds}) ===")
    for k in keys:
        s = summary[k]
        if "mean" in s:
    ...
    marker_dir = BENCH / "results" / "r1" / "ml20m"
    ...
    out_path = RESULTS_DIR / "r1_metrics.json"
