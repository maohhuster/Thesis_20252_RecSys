#!/usr/bin/env python3
"""Evaluate R1 (RLMRec-gene) checkpoints on Amazon-Books-2018 test set.
...
CK_DIR = BENCH / "checkpoints_amazon" / "r1" / "bge-large-en-v1.5"
...
def main():
    ...
    print("\n=== R1 (RLMRec-gene) on Amazon-Books-2018 ===")
    for k in keys:
        s = summary[k]
        print(f"  {k:10s} {s['mean']:.4f} ± {s['std']:.4f}")

    out_path = RESULTS_DIR / "r1_amazon_metrics.json"
