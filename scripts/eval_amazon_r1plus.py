...
Usage:
    python3 scripts/eval_amazon_r1.py
    python3 scripts/eval_amazon_r1.py --layer-num 2 --device cpu
"""
...
CK_DIR = BENCH / "checkpoints_amazon" / "r1plus" / "bge-large-en-v1.5"
...
def main():
    ...
    ap.add_argument("--layer-num", type=int, default=2,
                    help="Matches the upstream Amazon `layer_num: 2` setting "
                         "we copied into the amazon_ours: YAML block.")
    ...
    print("\n=== R1-plus (RLMRec-plus) on Amazon-Books-2018 ===")
    for k in keys:
        s = summary[k]
        print(f"  {k:10s} {s['mean']:.4f} ± {s['std']:.4f}")

    out_path = RESULTS_DIR / "r1plus_amazon_metrics.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(
        {"layer_num": args.layer_num, "per_seed": per_seed, "summary": summary},
