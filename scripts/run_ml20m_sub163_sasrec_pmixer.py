#!/usr/bin/env python3
"""Run pmixer's exact SASRec model on ML-20M with FULL-RANKING eval.

Mirrors `scripts/run_amazon_sasrec_pmixer.py` but with pmixer's published
ML-1M / dense-MovieLens hyperparameters (Result_Norm.md):

    embed_dim    = 50
    num_blocks   = 2
    num_heads    = 1
    dropout      = 0.2      ← ML-1M / ML-20M default; sparse Amazon would use 0.5
    maxlen       = 200      ← ML default; sparse Amazon would use 50
    lr           = 1e-3
    batch_size   = 128
    num_epochs   = 1000     (early-stopped via patience)
    l2_emb       = 0.0
    optimizer    = Adam, betas=(0.9, 0.98)
    norm_first   = False    ← pmixer default (Post-LN, original SASRec design)

Evaluator: our `evaluate.compute_metrics` for full-ranking NDCG@10 /
Recall@10 / MRR over all 9,906 items, on our ML-20M temporal val/test splits
(matching the protocol used by M1/M4/M7/R1/R2 in the paper).

Usage:
    python3 scripts/run_ml20m_sasrec_pmixer.py --seed 42 --device cuda
"""
...
DATA_DIR = BENCH / "data" / "processed_ml20m_sub163"           # ML-20M (default benchmark layout)
RESULTS_DIR = BENCH / "results_ml20m_sub163"
CHECKPOINT_DIR = BENCH / "checkpoints"

sys.path.insert(0, str(PMIXER))
sys.path.insert(0, str(BENCH))
from model import SASRec  # noqa: E402   ← pmixer unmodified
from evaluate import compute_metrics  # noqa: E402

# pmixer's published ML hyperparameters (Result_Norm.md: dropout=0.2 maxlen=200 for MovieLens)
HPARAMS = SimpleNamespace(
    hidden_units=50,
    num_blocks=2,
    num_heads=1,
    dropout_rate=0.2,         # ← ML default
    maxlen=200,               # ← ML default
    lr=1e-3,
    batch_size=128,
    num_epochs=1000,
    l2_emb=0.0,
...
logger = logging.getLogger("sasrec_pmixer_ml20m")
...
def load_data():
    """Same logic as Amazon wrapper but reading from processed/ (ML-20M).
    Returns user_train, val_gt, test_gt, n_users, n_items in pmixer's 1-indexed space.
    """
    def _load(name):
        return pd.read_csv(DATA_DIR / f"{name}.csv").sort_values(["userId", "timestamp"])

    train = _load("train"); val = _load("val"); test = _load("test")
...
def sample_one(uid, user_train, n_items, maxlen):
    ...
    nxt = history[-1]; idx = maxlen - 1
    ts = set(history)
    for i in reversed(history[:-1]):
        seq[idx] = i; pos[idx] = nxt
...
def full_ranking_eval(model, user_train, eval_gt, n_items, maxlen, device):
    ...
    for start in range(0, len(eval_uids), bs):
        ...
        for j, u in enumerate(bu):
            tail = user_train.get(u, [])[-maxlen:]
            seqs_np[j, maxlen - len(tail):] = tail
        logits = model.predict(np.asarray(bu), seqs_np, all_items).cpu().numpy()
        for j, u in enumerate(bu):
            gt = eval_gt[u]
            if not gt: continue
            train_items_0idx = {i - 1 for i in user_train.get(u, [])}
            gt_0idx = {i - 1 for i in gt}
            m = compute_metrics(logits[j], gt_0idx, train_items_0idx, n_items, TOP_K)
            for k, v in m.items():
                metrics[k].append(v)
...
def train(args):
    ...
    set_seed(args.seed)
    user_train, val_gt, test_gt, n_users, n_items = load_data()
    logger.info(f"Loaded: n_users={n_users}, n_items={n_items}, "
                f"train_users={len(user_train)}, "
                f"avg_seq={np.mean([len(h) for h in user_train.values()]):.1f}")
    ...
    ck_dir = CHECKPOINT_DIR / exp_name; ck_dir.mkdir(parents=True, exist_ok=True)
    res_dir = RESULTS_DIR / exp_name; res_dir.mkdir(parents=True, exist_ok=True)

    best_ndcg = 0.0; best_metrics = {}; best_epoch = 0
    ...
    for epoch in range(1, hp.num_epochs + 1):
        ...
        for u, seq, pos, neg in make_batches(user_train, n_items, hp.maxlen,
            ...
            loss += F.binary_cross_entropy_with_logits(neg_logits[indices], torch.zeros_like(neg_logits)[indices])
        ...
        if epoch % EVAL_EVERY == 0 or epoch == hp.num_epochs:
            t1 = time.time()
            val_metrics = full_ranking_eval(model, user_train, val_gt, n_items, hp.maxlen, device)
            ...
            logger.info(f"epoch {epoch:3d}  loss={avg_loss:.4f}  "
                        ...
                        f"({train_time:.0f}s+{eval_time:.0f}s)")
            ndcg10 = val_metrics.get("NDCG@10", 0)
            if ndcg10 > best_ndcg:
                best_ndcg = ndcg10; best_metrics = val_metrics; best_epoch = epoch
                patience_counter = 0
    ...
    test_metrics = full_ranking_eval(model, user_train, test_gt, n_items, hp.maxlen, device)
    logger.info(f"=== Test ===  NDCG@10={test_metrics['NDCG@10']:.4f}  "
                f"R@10={test_metrics['Recall@10']:.4f}  MRR={test_metrics['MRR']:.4f}")

    out = {
        "experiment": exp_name,
        "config": {**vars(HPARAMS), "seed": args.seed, "best_epoch": best_epoch,
                   "model_source": "pmixer/SASRec.pytorch (vendored)",
                   "evaluator": "our full-ranking compute_metrics",
                   "split": "temporal (processed)"},
        "best_val_metrics": best_metrics,
        "test_metrics": test_metrics,
    }
    (res_dir / "results.json").write_text(json.dumps(out, indent=2, default=str))
    logger.info(f"Saved {res_dir / 'results.json'}")
